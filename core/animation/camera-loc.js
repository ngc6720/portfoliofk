...
class CameraLoc {
  ...
  constructor(camera) {
    this.#camera =
      camera ?? console.error("provide a camera to CameraLoc constructor");
  ...
  update() {
    ...
    this.#positions.forEach((data, i) => {
      const coords = data
        .get(this.#scroll)[0]
        .map((coord) => coord * this.#coeffs[i]);
      pos[0] += coords[0];
      pos[1] += coords[1];
      pos[2] += coords[2];
      const coordslookat = data
        .get(this.#scroll)[1]
        .map((coord) => coord * this.#coeffs[i]);
  ...
  setPage(id) {
    ...
    this.#coeffAnim.to(
      this.#coeffs.map((coeff, i) => (coeff = i === id ? 1 : 0))
    );
  }
  initPage(id) {
