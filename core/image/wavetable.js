import { pipe } from "/core/utils/misc.js";
import {
  uni2bipo,
  bipo2uni,
  scale,
  ...
  const waveformTypes = [
    (phz, param) =>
      pipe(
        ...
        sin({
          phase: scale(param.index, 0, param.qt, 0, 0.5),
          period: param.rand2 * 4,
        }),
        tanh(0.6),
        (x) =>
          x +
          clip(
            -2,
            2
          )(sin({ phase: param.rand1 * 9, period: 2 })(phz.mod1 * 0.2) * 8),
        (x) =>
          x +
          clip(
            -4,
            4
          )(sin({ phase: param.rand1 * 9, period: 3 })(phz.mod1 * 0.5) * 2),
        (x) => ((x * 1) / param.qt) * 0.8,
...
function makePathes({ width = 512, height = 200, vMargin = 1, qt = 8 } = {}) {
  ...
  const cb = (pathIndex) => (pen, ramps) => {
    const y = waveformTypes[0](
      ramps,
      Object.assign(params, { index: pathIndex, qt: qt })
    );
