import { wrap, clip } from "/core/utils/math.js";
...
class Player extends AudioWorkletProcessor {
  ...
  constructor(options) {
    ...
    const fadeRampms = options?.processorOptions?.fadems
      ? options?.processorOptions?.fadems
      : 10;
    ...
    this.port.onmessage = ({ data }) => {
      ...
      //start
      if (data.play === 1) {
        if (this.#head >= this.#bufferL.length) this.#reset = 1;
        this.#play = 1;
      }
      //pause
  ...
  process(inputList, outputList, parameters) {
    for (let smp = 0; smp < outputList[0][0].length; smp++) {
      outputList[0][0][smp] =
        (this.#bufferL[this.#head] || 0) * parameters.gain[0] * this.#fadeRamp;
      outputList[0][1][smp] =
        (this.#bufferR[this.#head] || 0) * parameters.gain[0] * this.#fadeRamp;

      if (outputList[1].length > 0)
        outputList[1][0][smp] = this.#head / this.#bufferL.length; // norm pos in buffer for vis
